...
class DemandeReferentielFluxFixture extends Fixture implements DependentFixtureInterface
{
    ...
    public function load(ObjectManager $manager)
    {
        ...
        for ($i = 0; $i < 10; $i++) {
            ...
            $demande->setComposantSource($this->getRandom($composants));
            $demande->setComposantTarget($this->getRandom($composants));
            $demande->setType($faker->randomElement([
                DemandeReferentielFlux::AJOUT,
                DemandeReferentielFlux::RETRAIT]));
